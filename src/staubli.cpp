...
namespace staubli
{
	...
	auto createControllerStaubli()->std::unique_ptr<aris::control::Controller>
	{
		...
		for (aris::Size i = 0; i < 6; ++i)
		{
			...
			if (i == 0)
			{
				std::string xml_str =
					...
					"				<PdoEntry name=\"touch_probe\" index=\"0x60FF\" subindex=\"0x00\" size=\"16\"/>"
					"			</Pdo>"
					"		</SyncManager>"
					"		<SyncManager is_tx=\"true\">"
	...
	auto createModelStaubli()->std::unique_ptr<aris::dynamic::Model>
	{
	...
	auto update_staubli(aris::server::ControlServer &cs)->void
	{
		...
		mot.writePdo(0x2000, 0x00, state);
		mot.writePdo(0x2800, 0x00, state);
		mot.writePdo(0x3000, 0x00, state);
		mot.writePdo(0x3800, 0x00, state);
		mot.writePdo(0x4000, 0x00, state);
		mot.writePdo(0x4800, 0x00, state);
	}
...
int main(int argc, char *argv[])
{
    ...
    cs.resetController(staubli::createControllerStaubli().release());
	cs.resetModel(staubli::createModelStaubli().release());
    cs.resetPlanRoot(staubli::createPlanRoot().release());
    std::cout<<"start controller server"<<std::endl;
	//启动线程
	cs.start();

	cs.setRtPlanPostCallback(staubli::update_staubli);
	//Start Web Socket//
	cs.open();
