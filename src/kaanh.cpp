...
namespace kaanh
{
	...
	auto createControllerRokaeXB4()->std::unique_ptr<aris::control::Controller>
	{
		...
		for (aris::Size i = 0; i < 6; ++i)
		{
			...
			double pos_offset[6]
			{
                0.075517187683872,  -0.303016836083556,  0.985045881303604,   0.0283613113109543,   0.7938403840531,   -0.070040604036869
			...
			double max_pos[6]
			{
                170.0 / 360 * 2 * PI, 135.0 / 360 * 2 * PI,	65.0 / 360 * 2 * PI, 170.0 / 360 * 2 * PI, 120.0 / 360 * 2 * PI, 360.0 / 360 * 2 * PI,
			};
			double min_pos[6]
			{
                -170.0 / 360 * 2 * PI, -96.0 / 360 * 2 * PI, -195.0 / 360 * 2 * PI, -170.0 / 360 * 2 * PI, -120.0 / 360 * 2 * PI, -360.0 / 360 * 2 * PI
			};
			double max_vel[6]
			{
                440.0 / 360 * 2 * PI, 355.0 / 360 * 2 * PI, 440.0 / 360 * 2 * PI, 480.0 / 360 * 2 * PI, 450.0 / 360 * 2 * PI, 705.0 / 360 * 2 * PI,
			};
			double max_acc[6]
			{
                2200.0 / 360 * 2 * PI, 1800.0 / 360 * 2 * PI, 2200.0 / 360 * 2 * PI, 2400.0 / 360 * 2 * PI, 2250.0 / 360 * 2 * PI, 3525.0 / 360 * 2 * PI,
			};

            if(i==1)
            {
                std::string xml_str =
                    "<EthercatMotion phy_id=\"" + std::to_string(2) + "\" product_code=\"0x000002ec\""
                    " vendor_id=\"0x000002e1\" revision_num=\"0x00029002\" dc_assign_activate=\"0x0300\""
            ...
            {
                std::string xml_str =
                    "<EthercatMotion phy_id=\"" + std::to_string(1) + "\" product_code=\"0x000002ec\""
                    " vendor_id=\"0x000002e1\" revision_num=\"0x00029002\" dc_assign_activate=\"0x0300\""
                    " min_pos=\"" + std::to_string(min_pos[i]) + "\" max_pos=\"" + std::to_string(max_pos[i]) + "\" max_vel=\"" + std::to_string(max_vel[i]) + "\" min_vel=\"" + std::to_string(-max_vel[i]) + "\""
                    " max_acc=\"" + std::to_string(max_acc[i]) + "\" min_acc=\"" + std::to_string(-max_acc[i]) + "\" max_pos_following_error=\"0.1\" max_vel_following_error=\"0.5\""
                    " home_pos=\"0\" pos_factor=\"" + std::to_string(pos_factor[i]) + "\" pos_offset=\"" + std::to_string(pos_offset[i]) + "\">"
            ...
            else
            {
                ...
                std::string xml_str =
                    "<EthercatMotion phy_id=\"" + std::to_string(i) + "\" product_code=\"0x000002ec\""
                    " vendor_id=\"0x000002e1\" revision_num=\"0x00029002\" dc_assign_activate=\"0x0300\""
                    " min_pos=\"" + std::to_string(min_pos[i]) + "\" max_pos=\"" + std::to_string(max_pos[i]) + "\" max_vel=\"" + std::to_string(max_vel[i]) + "\" min_vel=\"" + std::to_string(-max_vel[i]) + "\""
                    " max_acc=\"" + std::to_string(max_acc[i]) + "\" min_acc=\"" + std::to_string(-max_acc[i]) + "\" max_pos_following_error=\"0.1\" max_vel_following_error=\"0.5\""
                    " home_pos=\"0\" pos_factor=\"" + std::to_string(pos_factor[i]) + "\" pos_offset=\"" + std::to_string(pos_offset[i]) + "\">"
        ...
        //set dc_mode to 0x300
        for(int i=0; i<6; i++)
        {
            dynamic_cast<aris::control::EthercatMotion&>(controller->slavePool().at(i)).setDcAssignActivate(0x300);
        }
        */
    ...
    auto ChangeIO::prepairNrt(const std::map<std::string, std::string> &params, PlanTarget &target)->void
    {
        ChangeIOParam param;
        param.output1 = 0x00;
        param.output2 = 0x00;
        ...
        {
            if (cmd_param.first == "output1")
            {
               param.output1 = std::stod(cmd_param.second);
            }
            else if (cmd_param.first == "output2")
            {
                param.output2 = std::stod(cmd_param.second);
            }
            else if (cmd_param.first == "input1")
            {
                param.input1 = std::stod(cmd_param.second);
            }
            else if (cmd_param.first == "input2")
            {
                param.input2 = std::stod(cmd_param.second);
            }
        }
        target.param = param;

    }
    auto ChangeIO::executeRT(aris::plan::PlanTarget &target)->int
    {
        auto &param = std::any_cast<ChangeIOParam&>(target.param);

        dynamic_cast<aris::control::EthercatSlave&>(target.controller->slavePool()[0]).writePdo(0x7000, 0x01, param.output1);
        dynamic_cast<aris::control::EthercatSlave&>(target.controller->slavePool()[0]).writePdo(0x7010, 0x01, param.output2);

        dynamic_cast<aris::control::EthercatSlave&>(target.controller->slavePool()[0]).readPdo(0x6000, 0x01, param.input1);
        dynamic_cast<aris::control::EthercatSlave&>(target.controller->slavePool()[0]).readPdo(0x6010, 0x01, param.input2);

        target.controller->mout()<<"0x"<<std::hex<<param.input1;
        target.controller->mout()<<"    0x"<<std::hex<<param.input2<<std::endl;

        return 0;
    }
    ChangeIO::ChangeIO(const std::string &name)
    {
        command().loadXmlStr(
            "<Command name=\"ch\">"
            "	<GroupParam>"
            "		<Param name=\"output1\" default=\"0\"/>"
            "		<Param name=\"output2\" default=\"0\"/>"
            "		<Param name=\"input1\" default=\"0\"/>"
            "		<Param name=\"input2\" default=\"0\"/>"
            "	</GroupParam>"
            "</Command>");
    }
	...
	auto createPlanRootRokaeXB4()->std::unique_ptr<aris::plan::PlanRoot>
	{
        ...
        plan_root->planPool().add<kaanh::ChangeIO>();
